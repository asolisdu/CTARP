...
loadjs.ready("head", function() {
	...
	faudittrailedit.lists["x_user"] = <?php echo $audittrail_edit->user->Lookup->toClientList($audittrail_edit) ?>;
	faudittrailedit.lists["x_user"].options = <?php echo JsonEncode($audittrail_edit->user->lookupOptions()) ?>;
	faudittrailedit.autoSuggests["x_user"] = <?php echo json_encode(["data" => "ajax=autosuggest"]) ?>;
...
<span<?php echo $audittrail_edit->id->viewAttributes() ?>><input type="text" readonly class="form-control-plaintext" value="<?php echo HtmlEncode(RemoveHtml($audittrail_edit->id->EditValue)) ?>"></span>
</span>
...
<?php if ($audittrail_edit->user->Visible) { // user ?>
	<div id="r_user" class="form-group row">
		<label id="elh_audittrail_user" class="<?php echo $audittrail_edit->LeftColumnClass ?>"><?php echo $audittrail_edit->user->caption() ?><?php echo $audittrail_edit->user->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<?php
$onchange = $audittrail_edit->user->EditAttrs->prepend("onchange", "");
$onchange = ($onchange) ? ' onchange="' . JsEncode($onchange) . '"' : '';
$audittrail_edit->user->EditAttrs["onchange"] = "";
?>
<span id="as_x_user">
	<input type="text" class="form-control" name="sv_x_user" id="sv_x_user" value="<?php echo RemoveHtml($audittrail_edit->user->EditValue) ?>" size="30" maxlength="80" placeholder="<?php echo HtmlEncode($audittrail_edit->user->getPlaceHolder()) ?>" data-placeholder="<?php echo HtmlEncode($audittrail_edit->user->getPlaceHolder()) ?>"<?php echo $audittrail_edit->user->editAttributes() ?>>
</span>
<input type="hidden" data-table="audittrail" data-field="x_user" data-value-separator="<?php echo $audittrail_edit->user->displayValueSeparatorAttribute() ?>" name="x_user" id="x_user" value="<?php echo HtmlEncode($audittrail_edit->user->CurrentValue) ?>"<?php echo $onchange ?>>
<script>
loadjs.ready(["faudittrailedit"], function() {
	faudittrailedit.createAutoSuggest({"id":"x_user","forceSelect":false});
});
</script>
<?php echo $audittrail_edit->user->Lookup->getParamTag($audittrail_edit, "p_x_user") ?>
</span>
