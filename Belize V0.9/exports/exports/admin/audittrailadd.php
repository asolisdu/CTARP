...
loadjs.ready("head", function() {
	...
	faudittrailadd.lists["x_user"] = <?php echo $audittrail_add->user->Lookup->toClientList($audittrail_add) ?>;
	faudittrailadd.lists["x_user"].options = <?php echo JsonEncode($audittrail_add->user->lookupOptions()) ?>;
	faudittrailadd.autoSuggests["x_user"] = <?php echo json_encode(["data" => "ajax=autosuggest"]) ?>;
...
<?php if ($audittrail_add->user->Visible) { // user ?>
	<div id="r_user" class="form-group row">
		<label id="elh_audittrail_user" class="<?php echo $audittrail_add->LeftColumnClass ?>"><?php echo $audittrail_add->user->caption() ?><?php echo $audittrail_add->user->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<?php
$onchange = $audittrail_add->user->EditAttrs->prepend("onchange", "");
$onchange = ($onchange) ? ' onchange="' . JsEncode($onchange) . '"' : '';
$audittrail_add->user->EditAttrs["onchange"] = "";
?>
<span id="as_x_user">
	<input type="text" class="form-control" name="sv_x_user" id="sv_x_user" value="<?php echo RemoveHtml($audittrail_add->user->EditValue) ?>" size="30" maxlength="80" placeholder="<?php echo HtmlEncode($audittrail_add->user->getPlaceHolder()) ?>" data-placeholder="<?php echo HtmlEncode($audittrail_add->user->getPlaceHolder()) ?>"<?php echo $audittrail_add->user->editAttributes() ?>>
</span>
<input type="hidden" data-table="audittrail" data-field="x_user" data-value-separator="<?php echo $audittrail_add->user->displayValueSeparatorAttribute() ?>" name="x_user" id="x_user" value="<?php echo HtmlEncode($audittrail_add->user->CurrentValue) ?>"<?php echo $onchange ?>>
<script>
loadjs.ready(["faudittrailadd"], function() {
	faudittrailadd.createAutoSuggest({"id":"x_user","forceSelect":false});
});
</script>
<?php echo $audittrail_add->user->Lookup->getParamTag($audittrail_add, "p_x_user") ?>
</span>
