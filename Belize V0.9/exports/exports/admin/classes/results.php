...
/**
 * Table class for results
 */
class results extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		// id
		$this->id = new DbField('results', 'results', 'x_id', 'id', '`id`', '`id`', 19, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// assessment_id
		$this->assessment_id = new DbField('results', 'results', 'x_assessment_id', 'assessment_id', '`assessment_id`', '`assessment_id`', 19, 11, -1, FALSE, '`assessment_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// created_at
		$this->created_at = new DbField('results', 'results', 'x_created_at', 'created_at', '`created_at`', CastDateFieldForLike("`created_at`", 0, "DB"), 135, 19, 0, FALSE, '`created_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->created_at->Sortable = TRUE; // Allow sort
		$this->created_at->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
		$this->fields['created_at'] = &$this->created_at;

		// updated_at
		$this->updated_at = new DbField('results', 'results', 'x_updated_at', 'updated_at', '`updated_at`', CastDateFieldForLike("`updated_at`", 0, "DB"), 135, 19, 0, FALSE, '`updated_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->updated_at->Sortable = TRUE; // Allow sort
		$this->updated_at->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
		$this->fields['updated_at'] = &$this->updated_at;

		// answers
		$this->answers = new DbField('results', 'results', 'x_answers', 'answers', '`answers`', '`answers`', 201, 65535, -1, FALSE, '`answers`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->answers->Sortable = TRUE; // Allow sort
		$this->fields['answers'] = &$this->answers;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`results`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->answers->DbValue = $row['answers'];
	}
	...
	{
		global $Language;
		if ($pageName == "resultsview.php")
			return $Language->phrase("View");
		elseif ($pageName == "resultsedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "resultsadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("resultsview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("resultsview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "resultsadd.php?" . $this->getUrlParm($parm);
		else
			$url = "resultsadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("resultsedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("resultsadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("resultsdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->answers->setDbValue($rs->fields('answers'));
	}
	...
	public function renderListRow()
	{
		...
		// answers
		$this->answers->ViewValue = $this->answers->CurrentValue;
		$this->answers->ViewCustomAttributes = "";
		...
		// answers
		$this->answers->LinkCustomAttributes = "";
		$this->answers->HrefValue = "";
		$this->answers->TooltipValue = "";
	...
	public function renderEditRow()
	{
		...
		// answers
		$this->answers->EditAttrs["class"] = "form-control";
		$this->answers->EditCustomAttributes = "";
		$this->answers->EditValue = $this->answers->CurrentValue;
		$this->answers->PlaceHolder = RemoveHtml($this->answers->caption());
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->answers);
				} else {
					$doc->exportCaption($this->id);
					$doc->exportCaption($this->assessment_id);
					$doc->exportCaption($this->created_at);
