...
class users extends DbTable
{
	...
	public function __construct()
	{
		...
		// id
		$this->id = new DbField('users', 'users', 'x_id', 'id', '`id`', '`id`', 19, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// first_names
		$this->first_names = new DbField('users', 'users', 'x_first_names', 'first_names', '`first_names`', '`first_names`', 200, 255, -1, FALSE, '`first_names`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->first_names->Sortable = TRUE; // Allow sort
		$this->fields['first_names'] = &$this->first_names;

		// last_names
		$this->last_names = new DbField('users', 'users', 'x_last_names', 'last_names', '`last_names`', '`last_names`', 200, 255, -1, FALSE, '`last_names`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->last_names->Sortable = TRUE; // Allow sort
		$this->fields['last_names'] = &$this->last_names;
		...
		// password
		$this->password = new DbField('users', 'users', 'x_password', 'password', '`password`', '`password`', 200, 255, -1, FALSE, '`password`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'PASSWORD');
		$this->password->Required = TRUE; // Required field
		$this->password->Sortable = TRUE; // Allow sort
		$this->fields['password'] = &$this->password;

		// user_level
		$this->user_level = new DbField('users', 'users', 'x_user_level', 'user_level', '`user_level`', '`user_level`', 3, 11, -1, FALSE, '`user_level`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "assessments") {
			$detailUrl = $GLOBALS["assessments"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
	...
	public function update(&$rs, $where = "", $rsold = NULL, $curfilter = TRUE)
	{
		...
		// Cascade Update detail table 'assessments'
		$cascadeUpdate = FALSE;
		$rscascade = [];
		if ($rsold && (isset($rs['id']) && $rsold['id'] != $rs['id'])) { // Update detail field 'user_id'
			$cascadeUpdate = TRUE;
			$rscascade['user_id'] = $rs['id'];
		}
		if ($cascadeUpdate) {
			if (!isset($GLOBALS["assessments"]))
				$GLOBALS["assessments"] = new assessments();
			$rswrk = $GLOBALS["assessments"]->loadRs("`user_id` = " . QuotedValue($rsold['id'], DATATYPE_NUMBER, 'DB'));
			while ($rswrk && !$rswrk->EOF) {
				...
				$success = $GLOBALS["assessments"]->Row_Updating($rsdtlold, $rsdtlnew);
				if ($success)
					$success = $GLOBALS["assessments"]->update($rscascade, $rskey, $rswrk->fields);
				if (!$success)
					return FALSE;

				// Call Row_Updated event
				$GLOBALS["assessments"]->Row_Updated($rsdtlold, $rsdtlnew);
				$rswrk->moveNext();
			}
		}
	...
	public function delete(&$rs, $where = "", $curfilter = FALSE)
	{
		...
		// Cascade delete detail table 'assessments'
		if (!isset($GLOBALS["assessments"]))
			$GLOBALS["assessments"] = new assessments();
		$rscascade = $GLOBALS["assessments"]->loadRs("`user_id` = " . QuotedValue($rs['id'], DATATYPE_NUMBER, "DB"));
		$dtlrows = ($rscascade) ? $rscascade->getRows() : [];

		// Call Row Deleting event
		foreach ($dtlrows as $dtlrow) {
			$success = $GLOBALS["assessments"]->Row_Deleting($dtlrow);
			if (!$success)
				break;
		}
		if ($success) {
			foreach ($dtlrows as $dtlrow) {
				$success = $GLOBALS["assessments"]->delete($dtlrow); // Delete
				if (!$success)
					break;
			}
		}
		...
		if ($success) {
			foreach ($dtlrows as $dtlrow)
				$GLOBALS["assessments"]->Row_Deleted($dtlrow);
		}
		if ($success)
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->first_names->DbValue = $row['first_names'];
		$this->last_names->DbValue = $row['last_names'];
	...
	public function getEditUrl($parm = "")
	{
		if ($parm != "")
			...
			$url = $this->keyUrl("usersedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	public function getCopyUrl($parm = "")
	{
		if ($parm != "")
			...
			$url = $this->keyUrl("usersadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->first_names->setDbValue($rs->fields('first_names'));
		$this->last_names->setDbValue($rs->fields('last_names'));
	...
	public function renderEditRow()
	{
		...
		$this->first_names->EditValue = $this->first_names->CurrentValue;
		...
		$this->last_names->EditValue = $this->last_names->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->first_names);
					$doc->exportCaption($this->last_names);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->first_names);
						$doc->exportField($this->last_names);
