...
class Api
{
	protected $SecretKey = 'ozxgDlXcCpdj5zMi';
		...
		$app->any('/[{params:.*}]', function (Request $request, Response $response, array $args) use ($api) {
			...
			if ($request->isOptions())
				return $response->withHeader("Access-Control-Allow-Origin", "*")
					->withHeader("Access-Control-Allow-Headers", "X-Requested-With, Content-Type, Access-Control-Allow-Headers, Accept, Origin, Authorization")
