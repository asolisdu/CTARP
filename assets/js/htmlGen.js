...
function buildCCPortraits(questions){
    ...
    questions.forEach((question, key, arr) => {
        ...
        if (question.title == 'Extreme Heat') {
            question.title = 'Drought'
        }
        if(key === arr.length - 1){
...
function buildCardView(lat, lon, address, question){
    ...
    let pt = ''
    if (question.title == 'Drought') {
        var extreme_heat = "Extreme Heat"
        pt = `<div class="shadow m-2" style="border-radius: 10px; display: none;" id="${question.title.replace(/\s/g, '')+'_container'}">
                    <div class="row">
                        <div class="col col-6">
                            <div class="embed-responsive embed-responsive-16by9" style="width: 100%; height: 600px;">
                                <iframe class="embed-responsive-item" 
                        ...
                        <div class="col my-auto">
                            <div class="row mb-4">
                                <div class="col text-center">
                                    <h2 class="d-inline-flex">${extreme_heat} Risk</h2>
