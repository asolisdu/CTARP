...
function getAssessmentsReportIndex(){
  $('#assessmentsReportTableBodyIndex').empty();
  var settings = {
      "url": apiURL+"?action=list&object=view1",
      "method": "GET",
      "timeout": 0,
  ...
  //   ////console.log(settings);
    ...
    $.ajax(settings).done(function (response) {
      ...
      response.view1.forEach((element, key, arr) => {
          ...
          `;
          $('#assessmentsReportTableBodyIndex').append(tableRow);
          if(key === arr.length - 1){
              var table = $('#assessmentsReportTableIndex').DataTable(
                {
                  dom: 'Bfrtip',
                  buttons: [
...
function buildReport(assessment_id, type){
  ...
  if(type == 'view'){
    ...
    customerLoanS = 'viewReportCustomerLoanS', customerRegion = 'viewReportCustomerRegion';
  ...
  $.ajax(settings).done(function (response) {
    ...
    console.log(response);
    console.log(response.fppi[0].id)
    var tbody = document.getElementById('tbody');
    document.getElementById('score').innerHTML = "Financial Product Price Index:  " + response.fppi[0].flat + " (From 0.1)"
    document.getElementById('fppi_name').innerHTML = "Client: " + response.assessment[0].customer_first_name + response.assessment[0].customer_last_name
    document.getElementById('fppi_sector').innerHTML = "Sector: " + response.loan_section[0].name
    for (var i = 0; i < response.fppi.length; i++) {
        var tr = "<tr>";


        /* Must not forget the $ sign */
        tr += "<td>" + response.fppi[i].name + "</td>" + "<td>" + response.fppi[i].flat + "</td>"+ "<td>" + response.fppi[i].scenario_1 + "</td>" + "<td>" + response.fppi[i].scenario_2 + "</td>" +
        "<td>" + response.fppi[i].scenario_3 + "</td>" + "</tr>";
    ...
    if(response){
      ...
      var Ggrade = response.scores.assessment_ratio;
      // calculating score
      // high > 66
      // medium > 33
      // low < 33
      //console.log('grade',Ggrade);
      let GgradeTxt = Ggrade <= 0.40 ? 'High' :  (0.80 >= Ggrade > 0.40) ? 'Medium' : Ggrade > 0.80 ? 'Low' : 'Nan';
      let GgradeColor = Ggrade <= 0.40 ? 'danger' :  (0.80 >= Ggrade > 0.40) ? 'warning' : Ggrade > 0.80 ? 'success' : 'Nan';
      ...
      response.section_scores.forEach((element, index, array) => {
        console.log(element.section_score)
        ...
        let gradeTxt = grade <= 0.60 ? 'High' :  (0.80 >= grade > 0.60) ? 'Medium' : grade > 0.80 ? 'Low' : 'Nan';
        console.log(grade)
        let gradeColor = grade <= 0.60 ? 'danger' :  (0.80 >= grade > 0.60) ? 'warning' : grade > 0.80 ? 'success' : 'Nan';
      ...
      // RISK MAPS
      $('#'+floodingMap).attr('src', `bzmaps/bzriskmap.html?lay=flood&lat=${response.assessment[0].lat}&lon=${response.assessment[0].lon}&z=10&pin=1&min=1&scalemax=4&color=Blues`);
      console.log(response.assessment[0].lat, response.assessment[0].lon)
      $('#'+wildfiresMap).attr('src', `bzmaps/bzriskmap.html?lay=fire&lat=${response.assessment[0].lat}&lon=${response.assessment[0].lon}&z=10&pin=1&min=1&scalemax=4&color=Oranges`);
      $('#'+droughtMap).attr('src', `bzmaps/bzriskmap.html?lay=drought&lat=${response.assessment[0].lat}&lon=${response.assessment[0].lon}&z=10&pin=1&min=1&scalemax=4&color=set1`);
      ...
      $('#'+customerRegion).text(response.assessment[0].region_name)
