...
$('#startAssessmentBtn').click(function() {
  console.log('current index start', $('.carousel-item.active').index());
  $(this).hide();
...
function validateCustomerInfo(){
  ...
  region_name = $('#customerRegionInput')
  ...
  if(region_name !== undefined) {
    $('#customerRegionError').hide();
  } else {
    allow = false;
    $('#customerRegionError').text('The field customer region is required');
    $('#customerRegionError').show();
  }
  // customer age
  ...
  setTimeout(() => {
    ...
    if(allow){
      ...
      payload.status = 0; 
      payload.region_name = region_name.val();
